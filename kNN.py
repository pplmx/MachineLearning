...
def classify(in_x, data_set, labels, k):
    ...
    sorted_class_count = sorted(
        class_count.items(), key=lambda obj: obj[1], reverse=True
    )
...
def dating_class_test():
    ...
    for i in range(num_test_data):
        classifier_result = classify(
            norm_arr[i, :],
            norm_arr[num_test_data:length, :],
            dating_labels[num_test_data:length],
            3,
        )
        print(
...
def classify_person():
    ...
    classifier_result = classify(
        (in_arr - min_vals) / ranges, norm_arr, dating_labels, 3
    )
...
def handwriting_class_test():
    ...
    for i in range(length):
        ...
        training_arr[i, :] = img2vector(
            "resource/digits/trainingDigits/%s" % filename_str
        )
    # 测试数据
    ...
    for i in range(length):
        ...
        classifier_result = classify(
            vector_under_test, training_arr, handwriting_labels, 3
        )
        print(
            "The classifier came back with: %d,the real answer is: %d"
