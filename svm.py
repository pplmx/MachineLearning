...
def simple_smo(data_list, class_label_list, constant, tolerate, max_loop):
    ...
    while loop < max_loop:
        ...
        for i in range(m):
            f_x_i = (
                float(multiply(alpha_mat, label_mat).T * (data_mat * data_mat[i, :].T))
                + b
            )
            err_i = f_x_i - float(label_mat[i])
            is_okay = (
                (label_mat[i] * err_i < -tolerate) and (alpha_mat[i] < constant)
            ) or ((label_mat[i] * err_i > tolerate) and (alpha_mat[i] > 0))
            if is_okay:
                j = select_j_rand(i, m)
                f_x_j = (
                    float(
                        multiply(alpha_mat, label_mat).T * (data_mat * data_mat[j, :].T)
                    )
                    + b
                )
                ...
                if (abs(alpha_mat[j]) - alpha_j_old) < 0.00001:
                    ...
                    continue
                alpha_mat[i] += (
                    label_mat[j] * label_mat[i] * (alpha_j_old - alpha_mat[j])
                )
                b1 = (
                    b
                    - err_i
                    - label_mat[i]
                    * (alpha_mat[i] - alpha_i_old)
                    * data_mat[i, :]
                    * data_mat[i, :].T
                    - label_mat[j]
                    * (alpha_mat[j] - alpha_j_old)
                    * data_mat[i, :]
                    * data_mat[j, :].T
                )
                b2 = (
                ...
                print(
                    "loop: %d, i: %d, pairs changed: %d"
                    % (loop, i, alpha_pairs_changed)
                )
...
def calc_err_k(o_s, k):
    f_x_k = float(multiply(o_s.alpha_mat, o_s.label_mat).T * o_s.K[:, k] + o_s.b)
...
def inner_l(i, o_s):
    ...
    is_okay = (
        (o_s.label_mat[i] * err_i < -o_s.tol) and (o_s.alpha_mat[i] < o_s.C)
    ) or ((o_s.label_mat[i] * err_i > o_s.tol) and (o_s.alpha_mat[i] > 0))
    if is_okay:
        ...
        if abs(o_s.alpha_mat[j] - alpha_j_old) < 0.00001:
            ...
            return 0
        o_s.alpha_mat[i] += (
            o_s.label_mat[j] * o_s.label_mat[i] * (alpha_j_old - o_s.alpha_mat[j])
        )
        update_err_k(o_s, i)
        ...
        b1_tmp1 = (
            o_s.b
            - err_i
            - o_s.label_mat[i] * (o_s.alpha_mat[i] - alpha_i_old) * o_s.K[i, i]
        )
        ...
        b2_tmp1 = (
            o_s.b
            - err_j
            - o_s.label_mat[i] * (o_s.alpha_mat[i] - alpha_i_old) * o_s.K[i, j]
        )
...
def platt_smo(
    data_list, class_label_list, constant, tolerate, max_loop, k_tup=("lin", 0)
):
    o_s = OptStruct(
        mat(data_list), mat(class_label_list).transpose(), constant, tolerate, k_tup
    )
    loop = 0
    entire_set = True
    alpha_pairs_changed = 0
    while (loop < max_loop) and ((alpha_pairs_changed > 0) or entire_set):
        ...
        if entire_set:
            for i in range(o_s.m):
                ...
                print(
                    "fullSet, loop: %d, i: %d, pairs changed %d"
                    % (loop, i, alpha_pairs_changed)
                )
            loop += 1
        else:
            non_bound_is = nonzero(
                (o_s.alpha_mat.A > 0) * (o_s.alpha_mat.A < constant)
            )[0]
            for i in non_bound_is:
                alpha_pairs_changed += inner_l(i, o_s)
                print(
                    "non bound, loop: %d, i: %d, pairs changed %d"
                    % (loop, i, alpha_pairs_changed)
                )
            loop += 1
        if entire_set:
